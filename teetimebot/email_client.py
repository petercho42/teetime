...
class EmailClient:
    ...
    def send_email_with_yahoo(receiver_email, subject, body):
        ...
        try:
            ...
            with smtplib.SMTP(EmailClient.YAHOO_SMTP_SERVER, EmailClient.YAHOO_SMTP_PORT) as server:
                server.starttls()
                server.login(EmailClient.YAHOO_SENDER_EMAIL, EmailClient.YAHOO_SENDER_PASSWORD)
    ...
    def send_email_with_outlook(receiver_email, subject, body):
        ...
        try:
            with smtplib.SMTP(EmailClient.OUTLOOK_SMTP_SERVER, EmailClient.OUTLOOK_SMTP_PORT) as server:
                server.starttls()
                server.login(EmailClient.OUTLOOK_SENDER_EMAIL, EmailClient.OUTLOOK_SENDER_PASSWORD)
                server.sendmail(EmailClient.OUTLOOK_SENDER_EMAIL, receiver_email, message.as_string())
            print("Email sent successfully.")
        except Exception as e:
